...
func NewSecp256k1IdentityFromHex(hex string) (*Secp256k1Identity, error) {
	priPr, err := crypto.HexToECDSA(hex)
	if err != nil {
		return nil, err
	}

	pri := &secp256k1.PrivateKey{
		PublicKey: priPr.PublicKey,
		D:         priPr.D,
	}

	return NewSecp256k1Identity(pri)
